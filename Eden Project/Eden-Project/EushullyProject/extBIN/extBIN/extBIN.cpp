// extBIN.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
long search_for_entry(int fd) {
	...
	if (memcmp(title, "SYS4415", 7) == 0 || memcmp(title, "SYS4472", 7) == 0) {
...
int retrieve_text(int fd, long entry, string filename) {
	...
	while (is_text(fd, next_entry)) {
		...
		fwprintf(output_file, L"●%08d●%s\n○%08d○%s\n\n", ++text_count, output_str.c_str(), text_count, output_str.c_str());
		next_entry += textlen;
		delete[] mb_str;
		delete[] wc_str;
