...
{
	lstrcpy(lpString, L"【枫笛汉化组】忘却管家与恋爱大小姐的回忆录 | 简体中文化补丁V1.0 | 禁止录播直播和商业行为");
...
void __stdcall AymGetFileName(wchar_t* FileName)
{
	if (lstrcmp(FileName, L".\\data2.dat") == 0)
		lstrcpy(FileName, L".\\koimemo.p");
}

void __stdcall AymReplaceXml(wchar_t* Text)
{
	//cout << wtoc(Text) << endl;
	if (lstrcmp(Text, L"Campus\\忘却執事と恋するお嬢様の回想録") == 0)
	{
		lstrcpy(Text, L"Campus\\忘却管家与恋爱大小姐的回忆录");
	}
}
...
{//00A8C580  朰媝幏帠偲楒偡傞偍忟條偺夞憐榐...............
	...
	//cout << wtoc(Text) << endl;
	if (lstrcmp(Text, L"朰媝幏帠偲楒偡傞偍忟條偺夞憐榐") == 0)
	{
		lstrcpy(Text, L"忘却管家与恋爱大小姐的回忆录");
	}
}
