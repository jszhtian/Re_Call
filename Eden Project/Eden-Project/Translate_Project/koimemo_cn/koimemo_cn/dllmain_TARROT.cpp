...
{
	lstrcpy(lpString, L"命运交织的塔罗牌Chs1.1中文版【白井木学园汉化组】参与汉化&交流&反馈bug请加群171900769");
...
void __stdcall GetText(wchar_t* Text)
{
	...
	AymReplaceTextW(Text, (wchar_t*)L"不運と幸運と恋占いのタロット", (wchar_t*)L"命运交织的塔罗牌");
}

void __stdcall AymGetFileName(wchar_t* FileName)
{
	if (lstrcmp(FileName, L".\\data2.dat") == 0)
		lstrcpy(FileName, L".\\cnpack.pk");
}

void __stdcall AymReplaceXml(wchar_t* Text)
{
	//cout << wtoc(Text) << endl;
	if (lstrcmp(Text, L"Campus\\不運と幸運と恋占いのタロット") == 0)
	{
		lstrcpy(Text, L"Campus\\命运交织的塔罗牌");
	}
}
...
{//00A8C580  朰媝幏帠偲楒偡傞偍忟條偺夞憐榐...............
	...
	//cout << wtoc(Text) << endl;
	if (lstrcmp(Text, L"晄塣偲岾塣偲楒愯偄偺僞儘僢僩") == 0)
	{
		lstrcpy(Text, L"命运交织的塔罗牌");
	}
}
...
void Hook()
{
	...
	//cout << "HookAddress:0x" << hex << pGetFileName << endl;
...
void __stdcall AymGetTextIdx(char* text,DWORD size)
{
	//TXT << "0x" << hex << idx << ","<< endl;
	//fix
	/*
	  0087F918  07 00 08 00 76 00 5F 00 61 00 6B 00 6E 00 30 00  v_akn0
	  0087F928  30 00 31 00 34 00 00 00 03 00 0A 00 0C 30 61 77  014..「睡
	  0087F938  2A 4E 24 95 50 5B 01 FF 01 FF 01 FF 0D 30 04 00  个锤子！！！」
	  0087F948  07 00 09 00 07 00 06                             .
	  000011c4h: 07 00 08 00 76 00 5F 00 61 00 6B 00 6E 00 30 00 ; ....v._.a.k.n.0.
	  000011d4h: 30 00 31 00 34 00 00 00 0C 30 61 77 2A 4E 24 95 ; 0.1.4....0aw*N$?
	  000011e4h: 50 5B 01 FF 01 FF 01 FF 0D 30 07 00 06 00       ; P[....0....

	  0087F919  00 0A 00 0C 30 B0 73 28 57 F2 5D CF 7E 30 52 86  ਀ఀ뀰⡳콝ま虒
	  0087F929  4E 31 00 30 00 74 5E 0E 54 86 4E 4A 55 01 FF 0D  ㅎ　琀๞虔䩎ŕ෿
	  0087F939  30 04 00 07 00 06 00                             а܀؀.

	  000018b0h: 07 00 0C 30 B0 73 28 57 F2 5D CF 7E 30 52 86 4E ; ...0皊(W騗蟸0R哊
	  000018c0h: 31 00 30 00 74 5E 0E 54 86 4E 4A 55 01 FF 0D 30 ; 1.0.t^.T哊JU..0
	  000018d0h: 06 00                                           ; ..
	*/
	if (memcmp(text, "\x07\x00\x08\x00\x76\x00\x5F\x00\x61\x00\x6B\x00\x6E\x00\x30\x00\x30\x00\x31\x00\x34\x00\x00\x00\x03\x00\x0A\x00\x0C\x30\x61\x77\x2A\x4E\x24\x95\x50\x5B\x01\xFF\x01\xFF\x01\xFF\x0D\x30\x04\x00\x07\x00\x09\x00\x07\x00\x06\x00", size)==0)
	{
		if (text[size + 1] == 0x00 && text[size + 2] == 0x00) 
		{
			memset(text, 0, size);
			char* fix = (char*)"\x07\x00\x08\x00\x76\x00\x5F\x00\x61\x00\x6B\x00\x6E\x00\x30\x00\x30\x00\x31\x00\x34\x00\x00\x00\x03\x00\x1e\x00\x0C\x30\x61\x77\x2A\x4E\x24\x95\x50\x5B\x01\xFF\x01\xFF\x01\xFF\x0D\x30\x07\x00\x06\x00";
			memcpy(text, fix, 46);
			size = 46;
		}
	}
...
PVOID pGetTextIdx = NULL;
__declspec(naked)void GetTextIdx()
{
	__asm
	{
		pushad
...
void DebugHook()
{
	pGetTextIdx = (PVOID)(BaseAddr + 0x27002D);
	DetourTransactionBegin();
	DetourAttach((void**)&pGetTextIdx, GetTextIdx);
	if (DetourTransactionCommit() != NOERROR)
	{
		MessageBox(NULL, L"ERROR", L"AymKze", MB_OK);
	}
