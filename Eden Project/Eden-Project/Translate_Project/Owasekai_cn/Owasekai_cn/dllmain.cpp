...
{
	const char* szWndName = "【伊甸学院】终末世界与生日 ｜ 简体中文预览补丁V0.1";
...
void __stdcall EdenGetText(char* Text)
{
	if (strlen(Text) != 0) 
	{
		cout << wtocGBK(ctowJIS(Text)) << endl;
		TXT << wtocUTF(ctowJIS(Text)) << endl;//あなたの誕生日を入力してください
		if (strcmp(wtocGBK(ctowJIS(Text)), "あなたの誕生日を入力してください") == 0)
		{
			strcpy(Text, "请输入你的生日。");
		}
	}
...
PVOID pGetMajiroText = NULL;
__declspec(naked) void GetMajiroText()
{
	_asm
	{
		pushad
...
PVOID pGetMajiroSNSText = NULL;
__declspec(naked) void GetMajiroSNSText()
{
	_asm
	{
		pushad
		...
		popfd
		popad
		jmp pGetMajiroSNSText
	}
}


int  (*MajiroTextHook)(int String, int a2, LPCSTR lpString, int a4, int a5);
int __cdecl MyTextHook(int String, int a2, LPCSTR lpString, int a4, int a5)
{
	cout << lpString << endl;
	TXT << lpString << endl;
	return MajiroTextHook(String, a2, lpString, a4, a5);
}


void Hook2()
{
	*(DWORD*)&MajiroTextHook = BaseAddr + 0x1AFC0;
	DetourTransactionBegin();
	DetourAttach((void**)&MajiroTextHook, MyTextHook);
	...
	* (DWORD*)& mEdenTextHook = BaseAddr + 0x7C7A0;
	...
	pGetMajiroText = (PVOID)(BaseAddr + 0x1F74F);
	pGetMajiroSNSText = (PVOID)(BaseAddr + 0x1AB02);
	cout << hex << BaseAddr << endl;
	cout << hex << pGetMajiroText << endl;
	cout << hex << pGetMajiroSNSText << endl;
	DetourTransactionBegin();
	DetourAttach((void**)&pGetMajiroText, GetMajiroText);
	DetourAttach((void**)&pGetMajiroSNSText, GetMajiroSNSText);
	DetourTransactionCommit();
	*/
}
