...
{
	wchar_t newtitle[] = L"【白井木学园汉化组】11月的理想乡 | 简体中文化补丁V1.0";
...
DWORD FileSize = NULL;
char* FileBuff = NULL;
void __stdcall AymReadFile(void* buff, DWORD size)
{
	if (Name && FileSize && FileBuff)
	{
		memset(buff, 0x00, size);
		memcpy(buff, FileBuff, FileSize);
		free(FileBuff);
		FileBuff = NULL;
		FileSize = NULL;
		Name = NULL;
...
void __stdcall AymOpenFile(char* name)
{
	if (name)
	{
		string fnm(name);
...
void FileSystemInit()
{
	pGetName = (PVOID)(BaseAddr + 0x6d9ce);
	pGetFileBuff = (PVOID)(BaseAddr + 0x6d768);
	pAllocMem = (PVOID)(BaseAddr + 0x6db4F);
	pAllocMem2 = (PVOID)(BaseAddr + 0x6db82);
	cout << hex << pGetName << endl;
	cout << hex << pGetFileBuff << endl;
	cout << hex << pAllocMem << endl;
	DetourTransactionBegin();
	DetourAttach((void**)&pGetName, GetName);
	DetourAttach((void**)&pGetFileBuff, GetFileBuff);
	DetourAttach((void**)&pAllocMem, AllocMem);
	DetourAttach((void**)&pAllocMem2, AllocMem2);
	if (DetourTransactionCommit() != NO_ERROR)
	{
		MessageBoxW(NULL, L"HOOK ERROR!!!", L"AymKze", MB_OK | MB_ICONWARNING);
	}
