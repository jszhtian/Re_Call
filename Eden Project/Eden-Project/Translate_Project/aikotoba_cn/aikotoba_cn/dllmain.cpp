...
int WINAPI NEWMessageBoxW(_In_opt_ HWND    hWnd,
	...
	_In_     UINT    uType)
{
	if (lpCaption && !lstrcmpW(lpCaption, L"終了"))
...
{
	DWORD Result = 0;
	//cout << hex << uChar << "|" << (char*)uChar << endl;
	if (uChar == 0x3F)
	{
		...
		Result = ((PfuncGetGlyphOutlineW)g_pOldGetGlyphOutlineW)(hdc, (UINT)0x30FB, fuFormat, lpgm, cjBuffer, pvBuffer, lpmat2);
	...
	{
		Result = ((PfuncGetGlyphOutlineW)g_pOldGetGlyphOutlineW)(hdc, uChar, fuFormat, lpgm, cjBuffer, pvBuffer, lpmat2);
	}
...
void ReplaceImage(BYTE* pBuffer, unsigned int nWidth, unsigned int nHeight, char* FileName)
{
	...
	if (ReadPngFile(FileName, 0, &oPngData) && oPngData.pRgba)
	{
