...
bool PngFile::ReadPngFile(const char* pszFileName, PNG_DATA* pPngData, int* chn)
{
	...
	bool bSuc = ReadPngFile(fPng, pPngData, chn);
...
bool PngFile::ReadPngFile(FILE* pFile, PNG_DATA* pPngData, int* chn)
{
	if (!pFile || !pPngData)
	{
		return false;
