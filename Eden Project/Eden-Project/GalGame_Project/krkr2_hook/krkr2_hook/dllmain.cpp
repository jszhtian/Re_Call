...
//  55 8B EC 83 C4 D8 B8 ?? ?? ?? ?? 53 56 57 8D 7D FC 8B 5D 08 E8 ?? ?? ?? ?? 85 DB 75 53 BB ?? ?? ?? ?? EB 4C 83 3D ?? ?? ?? ?? ?? 74 08 FF 15 ?? ?? ?? ?? EB 3B 8D 45 D8 BA ?? ?? ?? ?? 50 6A 00 6A 00 6A 00 6A 01 68 ?? ?? ?? ?? 6A 00 B9 ?? ?? ?? ?? 66 C7 45 ?? ?? ?? 89 17 FF 45 F4 89 0F FF 45 F4 57 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 24 53 E8 ?? ?? ?? ?? 59 8B F0 85 C0 74 A7 8B C6 8B 55 D8 64 89 15 ?? ?? ?? ?? 5F 5E 5B 8B E5 5D C3
typedef void* (_cdecl* tKrkrCdeclNewProc)(size_t);
PVOID AddrOfKrkrNew = (PVOID)(BaseAddr + 0x1600E8);
tKrkrCdeclNewProc pfnKrkrNew = (tKrkrCdeclNewProc)AddrOfKrkrNew;

//  55 8B EC 83 C4 F4 53 8B 45 08 85 C0 74 0F 8B D8 8D 45 F4 E8 ?? ?? ?? ?? 8B C3 FF 55 F8 5B 8B E5 5D C3
typedef void(_cdecl* tKrkrCdeclFreeProc)(void*);
PVOID AddrOfKrkrFree = (PVOID)(BaseAddr + 0x1608CC);
...
public:
    ...
    {
        tjs_uint count = min((size_t)read_size, m_size - m_offset);
...
void HookMemoryStreamDestructorForKrkr2(PVOID pObj)
{
    ...
    WriteMemory(&pVftbl[5], &pfnDetour, sizeof(pfnDetour));
}
...
tTJSBinaryStream* _HookTVPCreateStream(ttstr* name, tjs_uint32 flags) {
    ...
    if (flags == TJS_BS_READ) {
        ...
        if (fp) {
            ...
            CMemoryStream* Obj = new CMemoryStream(FileBuff, FileSize);
            if (!FixFlag)
            {
                HookMemoryStreamDestructorForKrkr2(Obj);
                FixFlag = TRUE;
            }
            KrkrFree(FileBuff);
            return Obj;
            ...
            return TVPCreateStreamCallback(name, flags);
        }
        //*/
        /*
        if (!wcscmp(wfnm.c_str(), L"startup.tjs")) {
            cout << "copying" << endl;
            CMemoryStream* Obj = new CMemoryStream(STARTUP_TJS, 233);
            if (!FixFlag)
            {
                HookMemoryStreamDestructorForKrkr2(Obj);
                FixFlag = TRUE;
