...
void NutFunction::PrintOpcode(std::ostream& out, int pos, const Instruction& op) const
{
	...
	TXT << std::setw(3) << std::setfill('0') << pos << std::setfill(' ');
	out << "]  " << codeName << spaces(14 - strlen(codeName));
	TXT << "]  " << codeName << spaces(14 - strlen(codeName));
	//mWriteText((char*)codeName);

	out << std::setw(5) << (int)op.arg0 << "  ";
	TXT << std::setw(5) << (int)op.arg0 << "  ";

	switch (code)
	{
	case OP_LOAD:
		out << m_Literals[op.arg1];
		TXT << m_Literals[op.arg1];
		break;
	...
	case OP_DLOAD:
		...
		TXT << std::setw(5) << (int)op.arg2;
		out << "  " << m_Literals[static_cast<unsigned char>(op.arg3)];
		TXT << "  " << m_Literals[static_cast<unsigned char>(op.arg3)];
		break;

	case OP_LOADINT:
		out << std::setw(5) << op.arg1;
		TXT << std::setw(5) << op.arg1;
		break;

	case OP_LOADFLOAT:
		out << std::setw(5) << op.arg1_float;
		TXT << std::setw(5) << op.arg1_float;
		break;

	case OP_LOADBOOL:
		out << "  " << (op.arg1 ? "true" : "false");
		TXT << "  " << (op.arg1 ? "true" : "false");
		break;
	...
	case OP_GETK:
		...
		TXT << '(' << (int)op.arg2 << ")." << m_Literals[op.arg1].GetString() << "  ";
		out << std::setw(5) << (int)op.arg3;
		TXT << std::setw(5) << (int)op.arg3;
		break;
...
void NutFunction::GenerateBodySource(int n, std::ostream& out) const
{
	...
	if (g_DebugMode)
	{
		...
		TXT << indent(n) << "// Literals:" << std::endl;
		...
		TXT << indent(n) << "// Outer values:" << std::endl;
		for (vector<OuterValueInfo>::const_iterator i = m_OuterValues.begin(); i != m_OuterValues.end(); ++i)
		{
			...
			TXT << indent(n) << "//\t" << i->type << "  src=" << i->src << "  name=" << i->name << std::endl;
		}
		...
		TXT << indent(n) << "// Local identifiers:" << std::endl;
		for (vector<NutFunction::LocalVarInfo>::const_reverse_iterator i = m_Locals.rbegin(); i != m_Locals.rend(); ++i)
		{
			...
			TXT << indent(n) << "//   -" << i->name << spaces(10 - i->name.size())
				<< " // pos=" << i->pos << "  start=" << i->start_op << "  end=" << i->end_op << (i->foreachLoopState ? " foreach state" : "") << std::endl;
		}
		...
		TXT << indent(n) << "// Instructions:" << std::endl;
		...
		for (size_t i = 0; i < m_Instructions.size(); ++i)
		{
			...
			TXT << indent(n) << "// " << std::setfill(' ') << std::setw(5) << currentLine << "  ";
		...
		TXT << indent(n) << "// Decompilation attempt:" << std::endl;
	}
