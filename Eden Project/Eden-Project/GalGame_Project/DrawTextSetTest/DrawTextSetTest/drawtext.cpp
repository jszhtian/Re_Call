...
{
	HANDLE hfile = CreateFileA(fontname.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
charBitmap FreeType::GetCharBitmap(wchar_t wchar)
{
	...
	if (error)
	{
		MessageBox(NULL, L"Load_Char Error", L"Error", MB_OK);
	}
...
void GdipDrawer::ApplyEffect(TextEffect effect, TextColor color, float _d_pixwidth, float _radius)
{
	...
	{
		MessageBox(NULL, L"Effect not support!", L"Error", MB_OK);
		return;
	}
