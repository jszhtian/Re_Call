...
void __stdcall MyReadFile(char* FileName)
{
    wstring fnm(ctowJIS(FileName));
    auto fp = _wfopen(fnm.c_str(), L"rb");
    fseek(fp, 0, SEEK_END);
    auto size = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    ...
    if (buff)
    {
        ...
        MessageBox(0, L"Can't Read File 1.", 0, 0);
}

int (* sub_46CA80)(DWORD* a1, unsigned int* a2, int a3, __int64 a4);
int  BGI_Decompress_func(DWORD* a1, unsigned int* a2, int a3, __int64 a4)
{
    cout << a1 << endl;
    cout << a2 << endl;
    cout << a3 << endl;
    cout << a4 << endl;
    return sub_46CA80(a1, a2, a3, a4);
}

void MainInit()
{
    *(PDWORD)&sub_46CA80 = (BaseAddr + 0x6CA80);

    DetourTransactionBegin();
    DetourAttach(&(LPVOID&)sub_46CA80, BGI_Decompress_func);
    //DetourAttach(&(LPVOID&)hkaddr, Thunk3);
    DetourTransactionCommit();
}
