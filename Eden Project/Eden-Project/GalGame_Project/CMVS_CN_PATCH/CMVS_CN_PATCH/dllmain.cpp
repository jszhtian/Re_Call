...
void __stdcall ProcessPushStr(UINT str, UINT off)
{
    ...
    {
        lstrcpyA(CopyStr, "中文测试１２３ＡＢＣａｂｃ中文测试１２３ＡＢＣａｂｃ中文测试１２３ＡＢＣａｂｃ中文测试１２３ＡＢＣａｂｃ");
        str_off = (UINT)CopyStr - str;
    }
    else if (off == 0x999)
    {
    ...
    if (FileName != nullptr)
    {
        ...
        if (FileName[szFileName - 3] == 'p' && FileName[szFileName - 2] == 'b' && FileName[szFileName - 1] == '3')
        {
            ...
            */
            ...
            NakedMemory dib;
            if (!ReadPngToBmp(buff, size, &width, &height, &bit_depth, dib, NULL))
            {
                delete[] buff;
                return;
            }
...
bool WINAPI HookSetWindowTextA(HWND hw, LPCSTR lpString)
{
    ...
    {
        //cout << hex << (UINT)(lpString[i] & 0xFF) << endl;
