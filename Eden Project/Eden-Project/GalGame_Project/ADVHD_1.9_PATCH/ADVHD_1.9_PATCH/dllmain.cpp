...
void ReadFileFromDir(wstring FileName)
{
    ...
    cout << "txt_line:" << txt_line.size() << endl;
    cout << "idx_line:" << idx_line.size() << endl;
...
void ReadFileFromPack(wstring FileName)
{
    ...
    wstring fnm = FileName.substr(0, FileName.length() - 4);
    FileNameToLower(fnm);
    wstring aym = fnm + L".aym";
    DWORD Txt_size = 0;
    if (!CFPK.GetFileByName((WCHAR*)aym.c_str(), &Txt_buff, &Txt_size))
        return;
    AYMHeader* hdr = (AYMHeader*)(Txt_buff + 4);
    hdr->StrCount ^= *(DWORD*)Txt_buff;
    //cout << "line:0x" << hex << hdr->StrCount << endl;

    // idx
    CompressInfo* idx_info = (CompressInfo*)(Txt_buff + 4 + sizeof(AYMHeader));
    //cout << "IdxSize:0x" << hex << idx_info->UncompressSize << endl;
    //cout << "pk_IdxSize:0x" << hex << idx_info->CompressSize << endl;

    BYTE* idx = Txt_buff + 4 + sizeof(AYMHeader) + sizeof(CompressInfo);
    ...
    {
        WCHAR err[MAX_PATH] = { 0 };
        wsprintfW(err, L"%s idx区解压错误。", aym.c_str());
        MessageBoxW(0, err, L"错误", MB_OK | MB_ICONERROR);
        //ExitProcess(-1);
        delete[] Txt_buff;
        Txt_buff = nullptr;
        return;
    ...
    // res
    CompressInfo* res_info = (CompressInfo*)(Txt_buff + 4 + sizeof(AYMHeader) + sizeof(CompressInfo) + idx_info->CompressSize);
    //cout << "ResSize:0x" << hex << res_info->UncompressSize << endl;
    //cout << "pk_ResSize:0x" << hex << res_info->CompressSize << endl;

    BYTE* res = Txt_buff + 4 + sizeof(AYMHeader) + sizeof(CompressInfo) + idx_info->CompressSize + sizeof(CompressInfo);
    for (int i = 0; i < res_info->CompressSize; i++)
        res[i] ^= pResKey[i % 1024];
    ...
    {
        WCHAR err[MAX_PATH] = { 0 };
        wsprintfW(err, L"%s res区解压错误。", aym.c_str());
        MessageBoxW(0, err, L"错误", MB_OK | MB_ICONERROR);
        //ExitProcess(-1);
        delete[] Txt_buff;
        Txt_buff = nullptr;
    ...
    {
        char* str = (char*)upk_res + aidx[i].jmp_off;
...
{
    // wchar_t
    auto FileName = *(int*)(regs->esp + 0x8);
    auto ArcName = *(int*)(regs->esp + 0x4);
    if (ArcName && FileName)
    {
       // cout << "ReadFile: " << wtoc((WCHAR*)FileName, CP_ACP) << " from " << wtoc((WCHAR*)ArcName, CP_ACP) << endl;
        auto fnm = wstring((WCHAR*)FileName);
        FileNameToLower(fnm);
