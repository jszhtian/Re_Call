...
int main(int argc, char* argv[])
{
	...
	do
	{
		if (start_buffer[i] == 0x0000007F)
		{
			firstOffset = *(DWORD*)&start_buffer[i + 4];
			break;
		}
		i += 4;
	} while (i < start_size);	
	...
	for (; pos < start_size; pos += 4)
	{
		...
		{
			DWORD TextOffset = *(DWORD*)&start_buffer[pos + 4];
			if (TextOffset < firstOffset)
				continue;
			cout << "Find text at offset:0x" << hex << TextOffset << endl;
			char* Text = (char*)&start_buffer[TextOffset];
...
int main(int argc, char* argv[])
{
	...
	if (argc != 2)
	{
		MessageBox(0, L"packBGI.exe <input Script file>", L"AyamiKaze", 0);
		return -1;
	}
	char* FileName = argv[1];
#endif
