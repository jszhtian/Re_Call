...
int main()
{
	...
	constexpr int FONT_WIDTH = 30;
	constexpr int FONT_HEIGHT = 30;

	error = FT_Set_Pixel_Sizes(face, FONT_WIDTH * 0.5, FONT_HEIGHT * 0.5);
	if (error)
	{
	...
	for (uint32_t y = 0; y < bitmap.rows; y++)
	{
		for (uint32_t x = 0; x < bitmap.width; x += 2)
		{
			...
			// 将像素值范围从0~255缩放到0~15
			pixel0 = pixel0 / 255.0 * 15.0;
			pixel1 = pixel1 / 255.0 * 15.0;

			// 设置输出像素值
			set_pixel(offset_x + x + 0, offset_y + y, pixel0);
			set_pixel(offset_x + x + 1, offset_y + y, pixel1);
		}
	}
