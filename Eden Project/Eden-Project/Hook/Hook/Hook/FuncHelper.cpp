...
bool PatchMemory(PatchStruct* psts, uint32_t cnt)
{
    for (size_t i = 0;i < cnt;i++)
    {
        ...
        if (!mod)
        {
            ...
            if (!mod)
            {
                printf("Patch: Can't find module: %s", st.mod_name);
                return false;
            }
        }
...
bool HookFunctions(const HookPointStruct* hooks, uint32_t cnt)
{
    ...
    for (size_t i = 0;i < cnt;i++)
    {
        ...
        if (!mod)
        {
            ...
            if (!mod)
            {
                printf("Hook: Can't find module: %s", hook->module_name);
                return false;
            }
        }
        ...
        {
            printf("Hook: Can't hook module: %s, offset: 0x%x", hook->module_name, hook->offset);
            return false;
        }
    }
    return true;
...
bool HookFunctions(const HookPointStructWithName* hooks, uint32_t cnt)
{
    ...
    for (size_t i = 0;i < cnt;i++)
    {
        ...
        if (!addr)
        {
            printf("Hook: Can't find %s in module: %s", hook->proc_name, hook->module_name);
            return false;
        }
        uint32_t hook_flag = hook->override_eax ? (STUB_DIRECTLYRETURN | STUB_OVERRIDEEAX) : 0;
        if (!InitializeHookSrcObject(&src, addr, true) ||
            ...
            !Hook32(&src, 0, &stub, hook->hook_routine, hook->reg_tags))
        {
            printf("Hook: Can't hook module: %s, name: %s", hook->module_name, hook->proc_name);
            return false;
        }
    }
    return true;
