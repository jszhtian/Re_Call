...
void Encrypt(char* FileName)
{
	...
	{
		buff[i] ^= XorTable[i % 1024];
		__int64 key = CRC64_String(&XorTable[i % 1024]);
		buff[i] += key;
		DWORD key2 = GetHashValue(&XorTable[i % 1024]);
		buff[i] ^= key2;
	}
	...
	fwrite(&f_header, sizeof(f_header), 1, fp);
...
void Decrypt(char* FileName)
{
	...
	fclose(fp);
	buff += 4;
	unsigned long size = *(unsigned long*)buff;
	buff += 4;
	cout << hex << size << endl;
	for (int i = 0; i < size; i++)
	{
