...
void PackFile(char *fname)
{
	...
	wchar_t bname[5];
	sprintf(pac_header.magic, "PAC\x00");
	...
	for (i = 0; i < pac_header.num; i++)
	{
		...
		{
			size_t ret_size = ZSTD_compress(cdata, Index[i].ComSize, udata, Index[i].FileSize, 1);
			if (ZSTD_isError(ret_size) == ZSTD_error_no_error)
			{
				Index[i].ComSize = ret_size;
				fwrite(cdata, 1, Index[i].ComSize, dst);
			}
			else
			{
				MessageBox(0, L"ZSTD compress error", L"Compress", 0);
				free(cdata);
				free(udata);
				fclose(src);
