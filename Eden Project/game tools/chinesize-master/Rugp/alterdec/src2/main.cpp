...
unordered_map<string, ClassFunction> ClassFuncTable;
...
int main( int argc, char **argv )
{
	try
	{
        ...
        //    0x20ecf,OpenRio);
		Decode("D:\\A_WorkSpace\\BLGame\\マブラヴオルタードフェイブル\\altfable.rio", 0x41f50/2, OpenRio2);
		//Decode( "D:\\Program Files\\マブラヴ14\\muvluv14.rio",
  //          0x335b6/2, OpenRio2);
		//Decode( "..\\alternative14.rio", 0x53a98/2, OpenRio2);

		FILE* fbb=fopen("a.b","wb");
		printf("sOffsets: 0x%08x\n", sOffsets.size());
...
{
    ObjectList.Add( "CrelicUnitedGameProject", 0, firstOffset, 0xFFFFFFFF );
...
{
	char* func_name = (char*)object->name.c_str();
	printf("--Reading object: %s\n", func_name);
	if ( object->size <= 3 )
		return;

	unordered_map<string,ClassFunction>::iterator it=ClassFuncTable.find(object->name);
...
//		{ "", ReadNull },
	...
	for(int i=0;i<sizeof(readfunc)/sizeof(funcPair);i++)
		ClassFuncTable.insert(unordered_map<string,ClassFunction>::value_type(readfunc[i].name,readfunc[i].func));
