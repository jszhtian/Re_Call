...
class HcbParser:
    ...
    def readstr(self):
        ret = 0
        code = self.hcb.read(self.hcb.readu8())
        code = code[:code.find(b'\x00')]
        try:ret = code.rstrip(b'\0').decode(self.codepage)
        except UnicodeDecodeError:
            print('UnicodeDecodeError:%x' % (self.hcb.tell()))
            sys.exit(0)
    ...
    def scanFunc2(self):
        ...
        while cur < self.codeBlockLen:
            inst = self.hcb.readu8()
            #print("current: %x || %x " % (inst, self.hcb.tell()))
            if inst > 0x27 or inst == 0:
                #print("current: %x || %x " % (inst, self.hcb.tell()))
                #int3()
                self.hcb.seek(1, 1)
            if inst == 1:
                ...
                try:self.hcb.seek(self.inst_len[inst], 1)
                except IndexError:
                    #print("OutInst:%x | %x" % (inst, self.hcb.tell()))
                    self.hcb.seek(1, 1)
                    #sys.exit(0)
            cur = self.hcb.tell()
            if cur >= j*0x50000:
