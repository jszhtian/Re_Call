...
static char* RunExeName;

typedef UINT (WINAPI* fnGetDriveTypeA)(
    LPCSTR lpRootPathName
    );
fnGetDriveTypeA GetDriveTypeAOLD;

UINT WINAPI NewGetDriveTypeA(
    LPCSTR lpRootPathName
)
{
    cout << lpRootPathName << endl;
    return 5;
}
...
void Lin()
{
    MessageBox(0, L"Crack By AyamiKaze[SWIFT@zdfx]", L"AyamiKaze", 0);
}
...
{
	//RunExeName = FileName;
    GetDriveTypeAOLD = (fnGetDriveTypeA)GetProcAddress(GetModuleHandle(L"kernel32.dll"), "GetDriveTypeA");
    DetourTransactionBegin();
    DetourAttach((void**)&GetDriveTypeAOLD, NewGetDriveTypeA);
	DetourAttach((void**)&g_pOldCreateFileA, NewCreateFileA);
    if (DetourTransactionCommit() != NO_ERROR)
    {
        MessageBoxW(NULL, L"HookError", L"Init", MB_OK | MB_ICONWARNING);
    }
    Lin();
}
...
BOOL NTAPI DllMain( HMODULE hModule,
