...
{
	VMProtectBeginVirtualization("Proc3");
	...
	iPos = Offset.LowPart % sizeof(GetKareseka()->m_HardwareCode);
	for (size_t i = 0; i < ReadSize; i++)
	{
		((PBYTE)ReadBuffer)[i] ^= GetKareseka()->m_HardwareCode[iPos % sizeof(GetKareseka()->m_HardwareCode)];
		iPos++;
	}
...
{
	//VMProtectBeginVirtualization("Proc2");
	...
	//hardware id code
	for (DWORD i = 0; i < ReadSize; i++)
		ReadBuffer[i] ^= GetKareseka()->m_HardwareCode[i % sizeof(GetKareseka()->m_HardwareCode)];
...
VOID StreamHolderXP3::Destroy()
{
	RtlZeroMemory(mRtcBuffer, mRtcBufferSize);

	for (ULONG i = 0; i < mRtcBufferSize; i++)
	{
		mRtcBuffer[i] = (BYTE)genrand64_int64();
	}
}
