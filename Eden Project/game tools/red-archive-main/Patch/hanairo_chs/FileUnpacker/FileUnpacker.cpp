...
int wmain(int argc, WCHAR* argv[])
{
	...
	HeaderInfo Header;
	File.Read((PBYTE)&Header, sizeof(HeaderInfo));
	Header.Count ^= CountXor;
	
	ChunkInfo* IndexBuffer = nullptr;
	ULONG IndexSize = sizeof(ChunkInfo) * Header.Count;

	IndexBuffer = (ChunkInfo*)HeapAlloc(GetProcessHeap(), 0, IndexSize);
	if (!IndexBuffer)
	{
		MessageBoxW(NULL, L"内存错误", nullptr, MB_OK);
		ExitProcess(-1);
	}
	File.Read((PBYTE)IndexBuffer, IndexSize);
	EncodeOrDecode((PBYTE)IndexBuffer, IndexSize);

	for (ULONG i = 0; i < Header.Count; i++)
	{
		File.Seek(IndexBuffer[i].Offset, FILE_BEGIN);
		PBYTE InBuffer = nullptr;
		
		if (FAILED(OutFile.Open(IndexBuffer[i].lpFileName, WinFile::FileWrite)))
		{
			MessageBoxW(NULL, IndexBuffer[i].lpFileName, L"无法写入", MB_OK);
			return 0;
		}

		InBuffer = (PBYTE)HeapAlloc(GetProcessHeap(), 0, IndexBuffer[i].Size);
		if (!InBuffer)
		{
			MessageBoxW(NULL, IndexBuffer[i].lpFileName, L"无法分配内存", MB_OK);
			return 0;
		}
		File.Read(InBuffer, IndexBuffer[i].Size);

		OutFile.Write(InBuffer, IndexBuffer[i].Size);
