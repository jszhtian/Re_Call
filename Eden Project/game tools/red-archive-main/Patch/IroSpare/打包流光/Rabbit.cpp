...
/* File name: rabbit.c                                                        */
/*----------------------------------------------------------------------------*/
/* Rabbit C source code in RaECRYPT format                                      */
/*----------------------------------------------------------------------------*/
...
/* Key setup */
void RaECRYPT_keysetup(RaECRYPT_ctx* ctx, const u8* key, u32 keysize, u32 ivsize)
{
	/* Temporary variables */
	u32 k0, k1, k2, k3, i;
...
/* IV setup */
void RaECRYPT_ivsetup(RaECRYPT_ctx* ctx, const u8* iv)
{
	/* Temporary variables */
	u32 i0, i1, i2, i3, i;
...
void RaECRYPT_process_bytes(int action, RaECRYPT_ctx* ctx, const u8* input,
...
/* Generate keystream */
void RaECRYPT_keystream_bytes(RaECRYPT_ctx* ctx, u8* keystream, u32 length)
{
	/* Temporary variables */
	u32 i;
	u8 buffer[16];
...
void RaECRYPT_process_blocks(int action, RaECRYPT_ctx* ctx, const u8* input,
...
#ifdef RaECRYPT_USES_DEFAULT_ALL_IN_ONE

/*
* Default implementation of all-in-one encryption/dRaECRYPTion of
* (short) packets.
*/
...
void RaECRYPT_process_packet(
	...
	u32 msglen)
{
	RaECRYPT_ivsetup(ctx, iv);

#ifdef RaECRYPT_HAS_SINGLE_BYTE_FUNCTION
	RaECRYPT_process_bytes(action, ctx, input, output, msglen);
#else
	if (action == 0)
		RaECRYPT_encrypt_bytes(ctx, input, output, msglen);
	else
		RaECRYPT_dRaECRYPT_bytes(ctx, input, output, msglen);
#endif
}
...
{
	RaECRYPT_ivsetup(ctx, iv);
	RaECRYPT_encrypt_bytes(ctx, plaintext, ciphertext, msglen);
}

void RaECRYPT_dRaECRYPT_packet(
	RaECRYPT_ctx* ctx,
	const u8* iv,
	const u8* ciphertext,
	u8* plaintext,
...
{
	RaECRYPT_ivsetup(ctx, iv);
	RaECRYPT_dRaECRYPT_bytes(ctx, ciphertext, plaintext, msglen);
}
