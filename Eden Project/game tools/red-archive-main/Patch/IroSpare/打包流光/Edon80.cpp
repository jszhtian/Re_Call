...
* Copyright 2005
*
* This code is placed as a reference code for Edon80ECRYPT
...
/* Here is the actual definition of Edon80ECRYPT_keysetup */
void Edon80ECRYPT_keysetup(Edon80ECRYPT_ctx* ctx, const u8* key, u32 keysize, u32 ivsize)
{
	...
	ctx->NumberOfInternalStates = Edon80ECRYPT_MAXKEYSIZE;
	...
	for (m = 0; m<keysize >> 1; m++)
	{
		...
		for (i = 0; i<4; i++)
			for (j = 0; j<4; j++)
			{
				...
				ctx->Q[m + Edon80ECRYPT_MAXKEYSIZE / 2][i][j] = Q[index][i][j];
			}
	}
}

/* Here is the actual definition of Edon80ECRYPT_ivsetup */
void Edon80ECRYPT_ivsetup(Edon80ECRYPT_ctx* ctx, const u8* iv)
{
	u32 i, j;        /* Variables i and j are internal counters. */
	s32 k;           /* Variable k is internal signed counter. */
	u8 Temp[Edon80ECRYPT_MAXKEYSIZE];         /* Temp is a vector of bytes that will temporarily
	...
	*/
	for (k = i - 1; k >= 0; k--)
	{
		ctx->InternalState[0] = ctx->Q[i - 1 - k][Temp[k]][ctx->InternalState[0]];
		for (j = 1; j<ctx->NumberOfInternalStates; j++)
			ctx->InternalState[j] = ctx->Q[i - 1 - k][ctx->InternalState[j - 1]][ctx->InternalState[j]];
	}
...
/* Here is the actual definition of Edon80ECRYPT_encrypt_bytes */
void Edon80ECRYPT_encrypt_bytes(Edon80ECRYPT_ctx* ctx, const u8* plaintext, u8* ciphertext, u32 msglen)
{
	u32 i, j;       /* Variables i and j are internal counters. */
...
/* Here is the actual definition of Edon80ECRYPT_decrypt_bytes */
void Edon80ECRYPT_decrypt_bytes(Edon80ECRYPT_ctx* ctx, const u8* ciphertext, u8* plaintext, u32 msglen)
{
	u32 i, j;       /* Variables i and j are internal counters. */
	u8 X;           /* We will store produced byte from the keystream in X. */
...
/* Here is the actual definition of Edon80ECRYPT_keystream_bytes */
void Edon80ECRYPT_keystream_bytes(Edon80ECRYPT_ctx* ctx, u8* keystream, u32 length)
{
	u32 i, j;       /* Variables i and j are internal counters. */
	u8 X;           /* We will store produced byte from the keystream in X. */
