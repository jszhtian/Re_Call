...
/*
* No local speed testing when using the SoECRYPT mode.
*/
#undef SOSEMANUK_SPEED

/*
* If we are using the SoECRYPT API, then we rely on the SoECRYPT portability
* macros and types.
*/
...
#ifdef SOSEMANUK_SoECRYPT
void
SoECRYPT_keysetup(SoECRYPT_ctx *kc, const u8 *key, u32 keysize, u32 ivsize)
#else
/* see sosemanuk.h */
void
...
#ifdef SOSEMANUK_SoECRYPT
void
SoECRYPT_ivsetup(SoECRYPT_ctx *ctx, const u8 *iv)
#else
/* see sosemanuk.h */
void
...
*/
#if defined SOSEMANUK_SoECRYPT
static void
sosemanuk_internal(SoECRYPT_ctx *rc, u8 *dst)
...
/* see ecrypt-sync.h */
void
SoECRYPT_process_bytes(int action, SoECRYPT_ctx *ctx,
const u8 *input, u8 *output, u32 msglen)
{
	(void)action;

	while (msglen > 0) {
		unsigned char tbuf[SoECRYPT_BLOCKLENGTH];
		size_t len;
...
/* see ecrypt-sync.h */
void
SoECRYPT_keystream_bytes(SoECRYPT_ctx *ctx, u8 *keystream, u32 length)
{
	while (length > 0) {
		if (length >= SoECRYPT_BLOCKLENGTH) {
			sosemanuk_internal(ctx, keystream);
			keystream += SoECRYPT_BLOCKLENGTH;
			length -= SoECRYPT_BLOCKLENGTH;
		}
		else {
			unsigned char tbuf[SoECRYPT_BLOCKLENGTH];
...
/* see ecrypt-sync.h */
void
SoECRYPT_process_blocks(int action, SoECRYPT_ctx *ctx,
const u8 *input, u8 *output, u32 blocks)
{
	...
	while (blocks-- > 0) {
		...
		sosemanuk_internal(ctx, tbuf);
		xorbuf(input, tbuf, output, SoECRYPT_BLOCKLENGTH);
		input += SoECRYPT_BLOCKLENGTH;
		output += SoECRYPT_BLOCKLENGTH;
	}
}

/* see ecrypt-sync.h */
void
SoECRYPT_keystream_blocks(SoECRYPT_ctx *ctx, u8 *keystream, u32 blocks)
{
	while (blocks-- > 0) {
		sosemanuk_internal(ctx, keystream);
		keystream += SoECRYPT_BLOCKLENGTH;
...
#ifdef SOSEMANUK_SoECRYPT
	SoECRYPT_init();
	SoECRYPT_keysetup(&ctx, key, key_len * 8, iv_len * 8);
	SoECRYPT_ivsetup(&ctx, iv);
#if defined SOSEMANUK_TEST_ENCRYPT_BYTES
	memset(tmp, 0, sizeof tmp);
	SoECRYPT_encrypt_bytes(&ctx, tmp, tmp, sizeof tmp);
#elif defined SOSEMANUK_TEST_DSoECRYPT_BYTES
	memset(tmp, 0, sizeof tmp);
	SoECRYPT_decrypt_bytes(&ctx, tmp, tmp, sizeof tmp);
#elif defined SOSEMANUK_TEST_ENCRYPT_BLOCKS
	memset(tmp, 0, sizeof tmp);
	SoECRYPT_encrypt_blocks(&ctx, tmp, tmp, 2);
#elif defined SOSEMANUK_TEST_DSoECRYPT_BLOCKS
	memset(tmp, 0, sizeof tmp);
	SoECRYPT_decrypt_blocks(&ctx, tmp, tmp, 2);
#elif defined SOSEMANUK_TEST_KEYSTREAM_BLOCKS
	SoECRYPT_keystream_blocks(&ctx, tmp, 2);
#else
	SoECRYPT_keystream_bytes(&ctx, tmp, sizeof tmp);
#endif
#else
