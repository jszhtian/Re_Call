...
void Decode(PBYTE Buffer, ULONG Size, ULONG64 Hash)
{
#if 0
...
int wmain(int argc, WCHAR* argv[])
{
	...
	DWORD Offset;
	InFile.Read((PBYTE)&Offset, 4);
	Offset ^= 0x84F2DA63;

	InFile.Seek(Offset, FILE_BEGIN);
	ULONG ChunkSize = InFile.GetSize32() - Offset;
	PBYTE ChunkData = new BYTE[ChunkSize];
	InFile.Read(ChunkData, ChunkSize);
	...
	{
		wstring FileName((LPCWSTR)(ChunkData + Offset));

		wprintf(L"unpacking : %s\n", FileName.c_str());
		Offset += (FileName.length() + 1) * 2;
		DWORD FileOffset = *(PDWORD)(ChunkData + Offset);
		Offset += 4;
		DWORD Size = *(PDWORD)(ChunkData + Offset);
		Offset += 4;
		ULONG64 Hash = *(PULONG64)(ChunkData + Offset);
		Offset += 8;

		PBYTE Buffer = new BYTE[Size];

		WinFile File;
		File.Open(FileName.c_str(), WinFile::FileWrite);
		InFile.Seek(FileOffset, FILE_BEGIN);
		InFile.Read(Buffer, Size);
		Decode(Buffer, Size, Hash);
