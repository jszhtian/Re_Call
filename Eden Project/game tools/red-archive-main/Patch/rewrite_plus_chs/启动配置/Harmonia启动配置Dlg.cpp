...
void CHarmonia启动配置Dlg::OnBnClickedChsButton()
{
	...
	BOOL bRet = CreateProcessW(
		L"Rewrite+汉化版.exe",//不在此指定可执行文件的文件名
		NULL,//命令行参数
		NULL,//默认进程安全性
		NULL,//默认进程安全性
	...
	{
		MessageBoxW(L"游戏启动失败！", L"Rewrite+启动配置", MB_OK | MB_ICONERROR);
	}
}
...
void CHarmonia启动配置Dlg::OnBnClickedOriButton()
{
	...
	PROCESS_INFORMATION pi = { 0 };
	BOOL bRet = CreateProcessW(
		L"Rewrite+原版.exe",//不在此指定可执行文件的文件名
		NULL,//命令行参数
		NULL,//默认进程安全性
		NULL,//默认进程安全性
		FALSE,//指定当前进程内句柄不可以被子进程继承
...
void CHarmonia启动配置Dlg::OnBnClickedFontButton()
{
	...
	if (Index == LB_ERR)
	{
		MessageBoxW(L"你没有选择任何一个字体", L"Rewrite+启动配置", MB_OK | MB_ICONERROR);
	}
	else
	{
		CString FontName;
		...
		MessageBoxW(L"配置保存成功", L"Rewrite+启动配置", MB_OK);
	}
}
