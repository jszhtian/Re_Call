...
BOOL FASTCALL CheckAndCreateProcess()
{
	...
	return VMeCreateProcess(NULL, NULL, L"SiglusEngine.exe", L"harmonia_chs.dll", NULL, NULL, FALSE, NULL, NULL, NULL, &si, &pi, NULL);
}
...
VOID CDECL MainEntry()
{
	...
	if (!CheckAndCreateProcess())
		MessageBoxW(NULL, L"无法启动游戏，请检查游戏完整性\n并且确保已经将本补丁加入防毒软件的白名单。", L"[X'moe汉化组]游戏启动失败", MB_OK | MB_ICONERROR);
