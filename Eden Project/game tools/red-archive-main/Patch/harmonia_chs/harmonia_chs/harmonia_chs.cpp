...
)
{
	...
	if (IsKernel32(lptstrFilename))
		return StubGetFileVersionInfoSizeW(L"harmonia_chs.dll", lpdwHandle);
...
)
{
	...
	if (IsKernel32(lptstrFilename))
		return StubGetFileVersionInfoW(L"harmonia_chs.dll", dwHandle, dwLen, lpData);
...
)
{
	...
	{
		//PatchCodeViaJmp((PVOID)(ThisModule + 0x1B753B), FixWidth1);
		//PatchCodeViaJmp((PVOID)(ThisModule + 0x1B7549), FixWidth2);
		//1B7274
		PatchCodeNope((PBYTE)(ThisModule + 0x1B7274), 2);
		//1B7262
		//直接干掉查表
		PatchCodeNope((PBYTE)(ThisModule + 0x1B7262), 9);
		//干掉Check disk

		//mov bl, 1
		static BYTE Opcode[] = { 0xb3, 0x01 };
		PatchReplaceCode((PBYTE)(ThisModule + 0x1F62F0), Opcode, countof(Opcode));

		//mov eax, 1
		static BYTE Opcode2[] = { 0xb8, 0x01, 0x00, 0x00, 0x00 };
		PatchReplaceCode((PBYTE)(ThisModule + 0x1F62E2), Opcode2, countof(Opcode2));
		PatchOnlyOnce = TRUE;
	}
	...
	if (IsScenePck && Handle != INVALID_HANDLE_VALUE)
		AddHandle(Handle);
...
)
{
	...
	auto IsSceneChsPack = [](LPCWSTR FileName)->BOOL
	{
		...
		return StrCompareW(FileName + iPos, L"Scene.chs") == 0;
	};
