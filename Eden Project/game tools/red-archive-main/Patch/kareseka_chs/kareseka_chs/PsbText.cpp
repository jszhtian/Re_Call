...
MRESULT WINAPI GetText(LPFILE_INFO lpFileInfo, LPDWORD lpdwRInfo)
{
	...
	lpFileInfo->hStream->Stat(&Stat, STATFLAG_DEFAULT);

	DWORD nFileSize = Stat.cbSize.LowPart;
	lpFileInfo->hStream->Seek(Offset, FILE_BEGIN, NULL);
...
NTSTATUS FASTCALL MyReplaceText(LPCWSTR FileName)
{
	NTSTATUS            Status;
	...
	LOOP_ONCE
	{
		...
		{
			if (RtlCompareMemory(&ReadLine[0], UTFBOM, sizeof(UTFBOM)) == sizeof(UTFBOM))
				ReadLine = &ReadLine[3];
...
int WINAPI ExtractPsbText(IStream* Stream, PBYTE& OutBuffer, ULONG& OutSize, LPCWSTR FileName)
{
	...
	if (NT_FAILED(MyReplaceText(FileName)))
	{
		PrintConsoleA("Read failed\n");
		return E_ERROR;
	}
