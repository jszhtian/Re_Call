...
namespace Anz
{
	...
	{
		Anz::Swap(length_, str.length_);
		Anz::Swap(capacity_, str.capacity_);
		Anz::Swap(buffer_, str.buffer_);
	}
...
#define GET_NEXT_CONTINUATION_BYTE(ptr) *ptr; if ((unsigned char)*ptr < 0x80 || (unsigned char)*ptr >= 0xc0) return '?'; else ++ptr;
	...
	String& String::AppendWithFormatArgs(const char* formatString, va_list args)
	{
		...
		while (true)
		{
			...
			default:
				PrintConsoleA("Unsupported format specifier: '%c'", format);
				break;
			}
		}
	}
	...
	void WString::Resize(unsigned newLength)
	{
		...
		else
		{
			...
			if (buffer_)
			{
				...
				RtlCopyMemory(newBuffer, buffer_, copyLength * sizeof(wchar_t));
				delete[] buffer_;
			}
