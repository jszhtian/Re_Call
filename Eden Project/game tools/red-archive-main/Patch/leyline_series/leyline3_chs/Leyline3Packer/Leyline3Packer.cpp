...
int wmain(int argc, WCHAR* argv[])
{
	...
	WinFile File;
	if (File.Open(L"Leyline.bin", WinFile::FileWrite) != S_OK)
	{
	...
	for (auto it : FileNamePool)
	{
		...
		if (InFile.Open(FullName.c_str(), WinFile::FileRead) == S_OK)
		{
			...
			//CHuffman HaffComp;
			long CompLength = 0;
			//CompLength = lzss.Compress(FileBytes, InFile.GetSize32(), FileComp);
			//HaffComp.Encode(FileComp, CompLength, FileBytes, InFile.GetSize32());

			CompLength = LZ4_compress((char*)FileBytes, (char*)FileComp, InFile.GetSize32());

			/*
			PBYTE OldComp = FileComp;
			FileComp = FileBytes;
			CompLength = InFile.GetSize32();
			*/
