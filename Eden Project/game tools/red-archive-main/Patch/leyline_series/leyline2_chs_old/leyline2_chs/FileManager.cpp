...
{
	if (File.Open(L"Leyline2.data", WinFile::FileRead) != S_OK)
	{
	...
	PBYTE ChunkBufferComp = (PBYTE)CMem::Alloc(Header.ChunkCompSize);
	PBYTE ChunkBuffer = (PBYTE)CMem::Alloc(Header.ChunkOriSize);
	...
	CMem::Free(ChunkBufferComp);
	CMem::Free(ChunkBuffer);
...
bool FileManager::QueryFile(const char* FileName, PBYTE& Buffer, ULONG& Size)
{
	...
	PBYTE CompFileBuffer = (PBYTE)CMem::Alloc(it->FileCompSize);
	PBYTE FileBuffer = (PBYTE)CMem::Alloc(it->FileOriSize);
