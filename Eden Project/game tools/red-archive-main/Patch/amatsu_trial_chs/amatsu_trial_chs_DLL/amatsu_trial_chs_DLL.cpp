...
{
	static WCHAR* Result = L"X'moe-32Bit-Edition-Patch-For-Purple";
	return Result;
}
	...
	)
{
	...
	return CreateFontW(nHeight, nWidth, nEscapement, nOrientation,
		fnWeight, fdwItalic, fdwUnderline, fdwStrikeOut, SHIFTJIS_CHARSET,
...
NTSTATUS NTAPI Init()
{
	...
	do
	{
		...
		Sucess = MH_CreateHook(CheckFont1_Start, HookCheckFont1, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x04]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(CheckFont2_Start, HookCheckFont2, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x05]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(CheckFont3_Start, HookCheckFont3, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x06]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		//Sucess = MH_CreateHook(CheckFont4_Start, HookCheckFont4, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x07]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(CheckFont5_Start, HookCheckFont5, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x08]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(CalculateLength1_Start, HookCalculateLength1, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x09]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(CalculateLength2_Start, HookCalculateLength2, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x0A]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(CalculateLength3_Start, HookCalculateLength3, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x0B]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(CalculateLength4_Start, HookCalculateLength4, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x0C]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(DetouredLoadImage_Start, DetouredLoadImage, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x0D]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(LoadScript_Start, DetouredLoadScript, (LPVOID*)&OldLoadScript);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[0x0E]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(GetStringPtrStart_1, Cmvs_GetStringPtr2_1, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[TextPtr1]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_CreateHook(GetStringPtrStart_2, Cmvs_GetStringPtr2_2, NULL);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[TextPtr2]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}

		Sucess = MH_EnableHook(MH_ALL_HOOKS);
		if (Sucess != MH_OK)
		{
			MessageBoxW(NULL, L"补丁启动失败，可能部分功能不可用[Init]", L"错误", MB_OK | MB_ICONERROR);
			break;
		}
