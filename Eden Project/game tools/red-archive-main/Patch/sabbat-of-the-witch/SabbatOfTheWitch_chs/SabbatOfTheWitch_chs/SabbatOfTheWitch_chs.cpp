// SabbatOfTheWitch_chs.cpp : 定义应用程序的入口点。
//

#include "stdafx.h"
#include "SabbatOfTheWitch_chs.h"
#include "WinFile.h"
#include "Exe.h"
#include "CRC64.h"
                     ...
                     _In_ int       nCmdShow)
{
	...
	WCHAR wszPath[] = L"SabbatOfTheWitch.exe";
	WCHAR wszDll[] = L"xmoe_sabbat_chs.dll";


	WinFile File;
	WinFile DllFile;
	if (DllFile.Open(wszDll, WinFile::FileRead) != S_OK)
	{
		MessageBoxW(NULL, L"无法找到汉化文件（请检查杀软）：\nxmoe_sabbat_chs.dll", L"[X'moe汉化组]魔女的夜宴", MB_OK);
		ExitProcess(-1);
	}
	else
	{
	...
	if (File.Open(wszPath, WinFile::FileRead) != S_OK)
	{
		MessageBoxW(NULL, L"无法找到原始文件：\nSabbatOfTheWitch.exe", L"[X'moe汉化组]魔女的夜宴", MB_OK);
		ExitProcess(-1);
	}
	...
	INT64 NewHash = crc64(0, Buffer, File.GetSize32());
	File.Release();
	
	INT64 OldHash = crc64(0, ExeData, sizeof(ExeData));

	if (OldHash != NewHash)
	{
		MessageBoxW(NULL, L"损坏的exe文件", L"[X'moe汉化组]魔女的夜宴", MB_OK);
		ExitProcess(-1);
	}
	...
	si.cb = sizeof(si);
	ZeroMemory(&pi, sizeof(pi));
	if (FALSE == CreateProcessWithDllW(NULL, wszPath, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi, wszDll, NULL))
	{
		if (pi.hThread != nullptr && pi.hProcess != INVALID_HANDLE_VALUE)
		{
			TerminateProcess(pi.hProcess, -1);
		}
		MessageBoxW(NULL, L"启动游戏失败！", L"[X'moe汉化组]魔女的夜宴", MB_OK);
		ExitProcess(-1);
	}
	return 0;
}
