#include "stdafx.h"
#include "WinFile.h"
#include "lzss.h"
#include "CMem.h"
#include "lz4.h"

#pragma comment(lib, "LZ4.lib")

static ULONG StaticMagic = 0x91F2ED00;

typedef struct SWEncodeHeader
{
	ULONG Magic;
	ULONG DecompressLength;

}SWEncodeHeader, *pSWEncodeHeader;


unsigned char KeyInfo[1024] =
{
	
};



int _tmain(int argc, _TCHAR* argv[])
{
	if (argc != 2)
	{
		return 0;
	}
	WinFile File;
	WinFile FileOutput;
	//LZSS    LzssCompression;
	PBYTE   RtcFileBuffer = nullptr, CompressedBuffer = nullptr;
	ULONG   FileSize = 0, CompressedSize;
	if (File.Open(argv[1], WinFile::FileRead) != S_OK)
	{
		return 0;
	}
	FileSize = File.GetSize32();
	RtcFileBuffer = (PBYTE)CMem::Alloc(FileSize);
	CompressedBuffer = (PBYTE)CMem::Alloc(FileSize * 2);
	if (RtcFileBuffer == nullptr)
	{
		return 0;
	}
	File.Read(RtcFileBuffer, FileSize);
	File.Release();

	//CompressedSize = LzssCompression.Compress(RtcFileBuffer, FileSize, CompressedBuffer + sizeof(SWEncodeHeader));
	CompressedSize = LZ4_compress((char*)RtcFileBuffer, (char*)(CompressedBuffer + sizeof(SWEncodeHeader)), FileSize);

	SWEncodeHeader Header;
	Header.DecompressLength = FileSize;
	Header.Magic = StaticMagic;
	RtlCopyMemory(CompressedBuffer, &Header, sizeof(SWEncodeHeader));
	CompressedSize += sizeof(SWEncodeHeader);

	WCHAR OutputName[MAX_PATH] = { 0 };
	wsprintfW(OutputName, L"%s.out", argv[1]);
	if (FileOutput.Open(OutputName, WinFile::FileWrite) != S_OK)
	{
		CMem::Free(RtcFileBuffer);
		CMem::Free(CompressedBuffer);
		return 0;
	}

	ULONG iPos = 0;
	while (iPos < CompressedSize)
	{
		CompressedBuffer[iPos] ^= KeyInfo[iPos % 1024];
		iPos++;
	}

	FileOutput.Write(CompressedBuffer, CompressedSize);
	FileOutput.Release();
	CMem::Free(RtcFileBuffer);
	CMem::Free(CompressedBuffer);
	return 0;
}

