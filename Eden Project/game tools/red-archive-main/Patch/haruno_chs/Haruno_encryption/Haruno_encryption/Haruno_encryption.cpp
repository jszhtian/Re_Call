// Haruno_encryption.cpp : 定义控制台应用程序的入口点。
//
...

{
	...
	StringSource pubFile("CE46960177E3149A8C2672F72B04090DB7596983AF9B82DFC58C3DE5E9647DF3", true, new HexDecoder);

	RSAES_OAEP_SHA_Encryptor pub(pubFile);
	//RSAES_OAEP_SHA_Encryptor pub(string("CE46960177E3149A8C2672F72B04090DB7596983AF9B82DFC58C3DE5E9647DF3"));
...
int WideToUTF8(const wchar_t * lpWideStr, const char * lpUTF8Str, int nUTF8StrLen)
{
	int nRetLen = 0;

	nRetLen = ::WideCharToMultiByte(CP_UTF8, 0, lpWideStr, -1, NULL, 0, NULL, NULL);  //获取转换到UTF8编码后所需要的字符空间长度

	if (nUTF8StrLen < nRetLen)  //如果输出缓冲区长度不够则退出
	{
		return 0;
	}
	nRetLen = ::WideCharToMultiByte(CP_UTF8, 0, lpWideStr, -1, (char *)lpUTF8Str, nUTF8StrLen, NULL, NULL);  //转换到UTF8编码
...
int _tmain(int argc, TCHAR* argv[])
{
	...
	wstring outName = wstring(argv[1]) + L".co";
	...
	DWORD CRC32 = crc32(0, buff, size);



	//MessageBoxW(NULL, L"压缩失败", L"Harono_E", MB_OK);
	uLongf out_size_t = size * 2;
	byte* out_buff = new byte[out_size_t];

	LZSS lzss;
	unsigned long out_size = lzss.Compress((unsigned char*)buff, size, out_buff);

	tTVPXP3ExtractionFilterInfo info;
	info.SizeOfSelf = sizeof(info);
	info.Buffer = out_buff;
	info.Offset = 0;
	info.FileHash = CRC32;
	info.BufferSize = out_size;

	ExtractionFilter(&info);

	//Encode3(buff, size);

	DWORD e_in_size = size ^ 0x474D12F6;
	DWORD e_out_size = out_size ^ 0xCF129DE1;
	DWORD crc_out = CRC32 ^ 0x51FD3AE5;

	FILE* out = _wfopen(outName.c_str(), L"wb");
	fwrite(Sign, 1, 16, out);
	fwrite(&e_in_size, 1, 4, out);
	fwrite(&e_out_size, 1, 4, out);
	fwrite(&crc_out, 1, 4, out);
