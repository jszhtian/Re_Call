...
PVOID GetTVPCreateStreamCall()
{
	...
	Kareseka->IStreamAdapterVtable = 0x6E7B1C;
	Kareseka->StubHostAlloc        = (FuncHostAlloc)0x55EC7C;
	CallTVPCreateStreamCall        = (PVOID)0x5FA648;
...
size_t NTAPI TJS_mbstowcsInternal(tjs_char *pwcs, const tjs_nchar *s, size_t n)
{
...
ASM size_t HookTJS_mbstowcs(tjs_char *pwcs, const tjs_nchar *s, size_t n)
{
	INLINE_ASM
	{
		push ecx;
	...
	LOOP_ONCE
	{
		...
		for (auto it = iBegin; it != iEnd; it++)
		{
			if (!StrCompareW(FileNameLower.c_str(), it->FileName.c_str()))
			{
				...
				FileBuffer = (PBYTE)AllocateMemoryP(it->Size);
				if (!FileBuffer)
					break;

				FileSize = it->Size;
				Hash     = it->Hash;
				XP3PackFile.Read(FileBuffer, FileSize);
...
NTSTATUS KaresekaHook::InitFileSystemXP3()
{
	...
	LOOP_ONCE
	{
		Status = XP3PackFile.Open(L"exe.bin");
		if (NT_FAILED(Status))
			break;
...
PVOID FindCallRtlDispatchException_Pass1()
{
	...
	KiUserExceptionDispatcher = (PBYTE)Nt_GetProcAddress(GetNtdllHandle(), "KiUserExceptionDispatcher");
...
LONG STDCALL GetExceptionHandler(PEXCEPTION_POINTERS ExceptionPointers)
{
	if ((ULONG)ExceptionPointers->ExceptionRecord->ExceptionAddress != (ULONG)FVPStream)
		return EXCEPTION_CONTINUE_SEARCH;
...
//Context == NULL(win10)
BOOLEAN FASTCALL HookRtlDispatchException(PEXCEPTION_RECORD ExceptionRecord, PCONTEXT Context)
{
	...
	if (GetExceptionHandler(&ExceptionPointers) == EXCEPTION_CONTINUE_EXECUTION)
		return TRUE;

	return OldRtlDispatchException(ExceptionRecord, Context);
}

DWORD CallOffset = 0x005FF198;
...
inline PLDR_MODULE GetKrMovieLdr2()
{
...
BOOL IsValidDllWorkerThread2(PVOID ThreadStart)
{
	...
	LOOP_ONCE
	{
		...
		{
			LayerExMovieLdr = GetModuleLdr2(LayerExMovieHandle);
			if (!LayerExMovieLdr)
				break;
	...
	LOOP_ONCE
	{
		if (KrMovieHandle)
		{
			KrMovieLdr = GetModuleLdr2(KrMovieHandle);
			if (!KrMovieLdr)
				break;
	...
	)
{
	...
	if (ExceptionInfo->ContextRecord->Eip == (DWORD)FVPStream)
	{
		...
		{
			ExceptionInfo->ContextRecord->Eip = (DWORD)MyCreateIStream;
			return EXCEPTION_CONTINUE_EXECUTION;
		}
	}
	return EXCEPTION_CONTINUE_SEARCH;
...
iTVPFunctionExporter* WINAPI HookTVPGetFunctionExporter()
{
	...
	if (!InitOnce)
	{
		...
		if (RtlDispatchExceptionAddress)
		...
		INLINE_PATCH_DATA f[] =
		{
			...
			{ RtlDispatchExceptionAddress,   HookRtlDispatchException, (PVOID*)&OldRtlDispatchException     }
			//{ (PVOID)0x43ABE8,               HookTJS_mbstowcs,    NULL }
		};

		auto CHECK_INT = [](INT i)->ULONG
		{
			if (i >= 0)
				return (ULONG)i;
			else
		...
		InlinePatchMemory(f, RtlDispatchExceptionAddress ? countof(f) : CHECK_INT(countof(f) - 1));
		InitOnce = TRUE;
	}
	return Result;
}
...
)
{
	...
	if (!StrICompareW(FileName.c_str(), L"patch3.xp3", StrCmp_ToLower))
...
NTSTATUS InitHook()
{
	...
	*(FARPROC *)&pfTVPGetFunctionExporter = (FARPROC)Nt_GetProcAddress(hModule, "TVPGetFunctionExporter");
	...
	return InlinePatchMemory(p, countof(p));
}
...
BOOL KaresekaHook::Init(HMODULE hModule)
{
	...
	INLINE_PATCH_DATA p[] =
	{
		...
		{ CreateFontIndirectW, HookCreateFontIndirectW, (PVOID*)&StubCreateFontIndirectW }
	};
