...
{
public:
	static ULONG32 Make(const T &val)
	{
		const char *p = (const char*)&val;
		const char *plim = (const char*)&val + sizeof(T);
		register ULONG32 ret = 0;
		while (p < plim)
		{
			ret += *p;
			ret += (ret << 10);
		...
		ret ^= (ret >> 11);
		ret += (ret << 15);
		if (!ret) ret = (ULONG32)-1;
		return ret;
	}
};
//---------------------------------------------------------------------------
template <>
class tTJSHashFunc<wstring> // a specialized template of tTJSHashFunc for ttstr
{
public:
	static ULONG32 Make(const wstring &val)
	{
...
//---------------------------------------------------------------------------
template <>
class tTJSHashFunc<WCHAR *> // a specialized template of tTJSHashFunc for tjs_char
{
public:
	static ULONG32 Make(const WCHAR *str)
	{
		if (!str) return 0;
		ULONG32 ret = 0;
		while (*str)
...
//---------------------------------------------------------------------------
template <>
class tTJSHashFunc<CHAR *> // a specialized template of tTJSHashFunc for tjs_nchar
{
public:
	static ULONG32 Make(const CHAR *str)
	{
		if (!str) return 0;
		ULONG32 ret = 0;
		while (*str)
...
public:
	...
	static ULONG32 MakeHash(const KeyT &key)
	{
	...
	void AddWithHash(const KeyT &key, ULONG32 hash, const ValueT &value)
	{
	...
	ValueT * FindWithHash(const KeyT &key, ULONG32 hash) const
	{
		// find key ( hash )
		// return   NULL  if not found
#ifdef TJS_HS_DEBUG_CHAIN
	...
	bool FindWithHash(const KeyT &key, ULONG32 hash, const KeyT *& keyout, ValueT *& value) const
	{
		// find key
		// return   false  if not found
#ifdef TJS_HS_DEBUG_CHAIN
	...
	ValueT * FindAndTouchWithHash(const KeyT &key, ULONG32 hash)
	{
	...
	bool FindAndTouchWithHash(const KeyT &key, ULONG32 hash, const KeyT *& keyout, ValueT *& value)
	{
		// find key
		// return   false  if not found
		const element * elm = InternalFindWithHash(key, hash);
	...
	bool DeleteWithHash(const KeyT &key, ULONG32 hash)
	{
...
private:
	...
	const element * InternalFindWithHash(const KeyT &key, ULONG32 hash) const
	{
		// find key ( hash )
#ifdef TJS_HS_DEBUG_CHAIN
		hash = 0;
