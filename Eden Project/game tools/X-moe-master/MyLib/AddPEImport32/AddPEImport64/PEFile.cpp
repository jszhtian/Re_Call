...
bool PEFile::readImportTable() {
	ULONGLONG tableRVA = peHeaders.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress;
	ULONGLONG tableOffset = rvaToOffset(tableRVA);
	if (tableOffset == 0) {
		return false;
	}
	...
	while (true) {
		...
		while (true) {
			if ((importThunk->u1.Ordinal & IMAGE_ORDINAL_FLAG64) == IMAGE_ORDINAL_FLAG64) {
				importFunction->FunctionId = IMAGE_ORDINAL64(importThunk->u1.Ordinal);
			}
			else {
				ULONGLONG nameOffset = rvaToOffset(importThunk->u1.AddressOfData);
...
void PEFile::buildImportTable() {
	...
	ULONGLONG oldImportTableRVA = peHeaders.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress;
	ULONGLONG oldImportTableOffset = rvaToOffset(oldImportTableRVA);
	...
	while (importDll != NULL) {
		...
		while (importFunction != NULL) {
			...
			if (importFunction->FunctionId != 0) {
				importThunk.u1.Ordinal = importFunction->FunctionId | IMAGE_ORDINAL_FLAG64;
			}
			else {
...
//==============================================================================
ULONGLONG PEFile::rvaToOffset(ULONGLONG rva) {
	for (int i = 0; i < peHeaders.FileHeader.NumberOfSections; i++) {
...
//==============================================================================
ULONGLONG PEFile::offsetToRVA(ULONGLONG offset) {
	for (int i = 0; i < peHeaders.FileHeader.NumberOfSections; i++) {
