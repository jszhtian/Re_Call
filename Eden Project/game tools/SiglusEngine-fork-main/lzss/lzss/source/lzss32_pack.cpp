...
/*
	...
	lzss.ready((DWORD*)src_data, src_data_cnt, 1024*100);
...
void Clzss32_pack::ready(DWORD* src_data_, DWORD src_data_cnt_, DWORD break_out_cnt_)
{
	_src_data = src_data_;
	_src_data_cnt = src_data_cnt_;

	_sizeof_data_size = sizeof(DWORD);
	_window_size = 1 << LZSS32_INDEX_BIT_COUNT;
	_break_even = LZSS32_BREAK_EVEN;
	_length_bit_count = 16 - LZSS32_INDEX_BIT_COUNT;
...
// ＬＺＳＳ３２圧縮 プロセス
// ================================================================
bool Clzss32_pack::proc(BYTE* dst_buf_, DWORD dst_buf_size_)
{
	_dst_buf = dst_buf_;
...
// ****************************************************************
// ＬＺＳＳ３２圧縮 プロセス 圧縮データをバッファにコピーする
// ================================================================
bool Clzss32_pack::_proc_copy()
{
...
// ****************************************************************
// ＬＺＳＳ３２圧縮 プロセス 最後の圧縮データをバッファにコピーする
// ================================================================
bool Clzss32_pack::_proc_last_copy()
{
	_copy_pack_buf(true);
	if (dst_buf_need_size > 0)	{
		return false;	// バッファサイズが足りません
...
// ＬＺＳＳ３２圧縮 データ化
// ================================================================
bool Clzss32_pack::_make_pack_data()
{
	...
	if (_tree_find.matching_size <= _break_even)	{
		...
		BYTE *p = (BYTE *)(_src_data + _tree_find.src_data_index);
		_pack_data[_pack_data_count] = *p++;		_pack_data_count++;
		_pack_data[_pack_data_count] = *p++;		_pack_data_count++;
		_pack_data[_pack_data_count] = *p++;		_pack_data_count++;
	}
	else	{
...
// ****************************************************************
// ＬＺＳＳ３２圧縮 データをバッファにコピーする
// ================================================================
bool Clzss32_pack::_copy_pack_buf(bool last_flag)
{
	dst_buf_need_size = 0;
