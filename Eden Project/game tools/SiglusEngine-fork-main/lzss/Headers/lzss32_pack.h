...
public:
	void		ready(DWORD* src_data_, DWORD src_data_cnt_, DWORD break_out_cnt_);	// 準備
...
private:
	...
	BYTE		_pack_data[1 + (3 * 8)];	// 圧縮データ：最大 = flag(１ビット × ８) + coad(３バイト × ８)

	Clzss_tree_find<DWORD>	_tree_find;		// ツリー検索
