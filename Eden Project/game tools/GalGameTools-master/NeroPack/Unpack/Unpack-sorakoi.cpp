...
char* hash_get_script_name(uint32_t Name,uint32_t DataName)
{
	...
	char* name_array[] = {"story.txt","start.txt","titlelogo.txt","title.txt","story.txt","CS01_N_01.txt",
"CS01_N_02.txt",
"CS01_N_03.txt",
"CS01_N_04.txt",
"CS01_N_05.txt",
...
void export_file_reader(FILE* f,magic_header_t* header,char* export_path)
{
	...
	//get script name set
	if (!strcmp("patch101", res_name))
		strcpy(res_name,"script");
	if(strcmp(res_name,"script")==0)
	{
		cout << hex << header->top.parity_name << endl;
		uint32_t hashName = hash_string_func(header->top.parity_name,"CS01_C_02.txt");
		cout << hex << hashName << endl;
