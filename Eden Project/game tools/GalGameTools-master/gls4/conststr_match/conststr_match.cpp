...
void put_text(unsigned char* top_buffer,size_t top_size,unsigned int offset)
{
	...
	if(!txt)
	{
		sprintf(section_name,"output_text.txt");
		txt = fopen(section_name,"wb");
		fwrite("\xFF\xFE",2,1,txt);
	}
	
	fwprintf(txt,L"0x%08X|",offset);
...
void search_text()
{
	...
	unsigned char find_bytes[] = { 0x00, 0x02, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x06 };
	FILE* fp_image;
	long f_size;
	unsigned char* f_buf;
				...
				if(index_of_name < match_array.size() && index_of_name >= 19024)
				{
					...
					if(match_array[index_of_name].string_length >= 2)
					{
						...
						if(!wcsstr(match_array[index_of_name].string,L"\r\n") && 
							match_array[index_of_name].string[0] != L'жа' &&
							match_array[index_of_name].string[0] != L'Дѓ')
						{
