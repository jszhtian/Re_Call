...
/* 功能：将LCUI_Graph结构中的数据写入至png文件 */
{
	...
	/*
	fstream fst;
	string fnm = file_name + ".dmp";
	fst.open(fnm, ios::out | ios::binary);
	int Wcout = 0;
	if(graph->flag)
	{
		Wcout = 4 * graph->width * graph->height;
	}
	else
	{
		Wcout = 3 * graph->width * graph->height;
	}
	fst.write((char*)graph->rgba, Wcout);
	fst.close();
	*/
	/* initialize stuff */
