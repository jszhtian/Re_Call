        ...
        public bFile 读取b(Stream bFileStream)
        {
            ...
            if (jis.GetString(b文件.Hdr).Substring(0, 6) == "abmp11" || jis.GetString(b文件.Hdr).Substring(0, 6) == "abmp12")
            {
                ...
                if (jis.GetString(b文件.bData.Hdr).Substring(0, 8) == "abdata10" ||
                    ...
                    jis.GetString(b文件.bData.Hdr).Substring(0, 8) == "abdata15")
                {
                    buff = new byte[4];
                            ...
                            for (int i = 0; i < b图像.nData; i++)
                            {
                                    ...
                                    header == "abimgdat11")
                                {
                                    ...
                                    else if (header == "abimgdat15")
                                    {
                                         ...
                                         * 也有可能是0x12
                                         */
                                        byte[] bVersion1 = { 0x01, 0x00, 0x00, 0x00 };
                                        byte[] bVersion2 = { 0x02, 0x00, 0x00, 0x00 };
                                        var IsVer1 = bVersion1.SequenceEqual(b数据.Unkown15);
                                        var IsVer2 = bVersion2.SequenceEqual(b数据.Unkown15);
                                        if (IsVer1)
                                        {
                                            b数据.Unkown = new byte[0x12];
                                            bFileStream.Read(b数据.Unkown, 0, b数据.Unkown.Length);
                                        }
                                        else if (IsVer2)
                                        {
                                            b数据.Unkown = new byte[0x1E];
                                            bFileStream.Read(b数据.Unkown, 0, b数据.Unkown.Length);
                                        }
                                        else
                                        {
                                            throw (new Exception("abimgdat15: WTF"));
                                        }
                                    }
                                    else
                                    {
                ...
                {
                    throw (new Exception("不是标准的b文件2."));
                }
            }
            else
            {
                throw (new Exception("不是标准的b文件1."));
            }
