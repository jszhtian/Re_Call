...
BGIInsRecord RecordInfo[] = 
{
	...
	{ 0xfe, NULL, 0 },
	{ 0xff, NULL, 0 },
	{ 0x0140, "PrintMessage", 5 },
	{ 0x0141, "ClearMessage", 0 },
	{ 0x0142, "StartNewLine", 0 },
	{ 0x0143, "RegisterTextToHistory", 0 },
	{ 0x0144, "DontRegisterTextToHistory", 0 },
};
...
HRESULT GetInstructionInfo(PBYTE Buffer, ULONG& iPos, string& Name)
{
	...
	else if (Ins >= 0x140 && Ins <= 0x144)
	{
		std::cout << std::hex << Ins << std::endl;
		std::cout  << RecordInfo[LOBYTE(Ins)].Name[0] << std::endl;
		if (RecordInfo[LOBYTE(Ins)].Name[0] == 0)
		{
			CHAR Info[100] = { 0 };
			wsprintfA(Info, "function_%08x", Ins);
